...
/**
 * Send the "new newsletter subscriber" notification email to the business inbox.
 *
 * Same contract as {@link sendLeadNotification}: returns a result object instead
 * of throwing, so a failed email never fails the subscription (the subscriber is
...
export async function sendNewsletterNotification(subscriber: {
  ...
  const receivedAt = subscriber.createdAt ?? new Date().toISOString();
  ...
  const html = `
    <div style="font-family:system-ui,-apple-system,Segoe UI,Roboto,sans-serif;max-width:560px;margin:0 auto;color:#0f172a;">
      <h2 style="margin:0 0 16px;font-size:18px;">New newsletter subscription</h2>
      <p style="margin:0 0 16px;font-size:14px;">A new user subscribed to SmartGestion updates.</p>
      <table style="width:100%;border-collapse:collapse;font-size:14px;">
        ${rows
          .map(
            ([label, value]) => `
  ...
  try {
    const { data, error } = await getResend().emails.send({
      ...
      to: notificationTo,
      subject,
      // Lets the business reply straight to the subscriber.
