  ...
  return (
    <div className="min-h-screen bg-white dark:bg-dark-900">
      ...
      <div className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div className="absolute -top-32 right-1/3 h-96 w-96 rounded-full bg-[#1D6FD6]/8 blur-3xl" />
        <div className="absolute left-1/4 top-1/2 h-64 w-64 rounded-full bg-[#3FB8C4]/6 blur-3xl" />
      </div>
      ...
      <main className="mx-auto max-w-4xl px-4 py-16 sm:px-6 lg:py-24">
        ...
        <div className="mb-10 grid gap-4 sm:grid-cols-3">
          {p.highlights.map((h) => (
            <div key={h.title} className="rounded-2xl border border-slate-200 bg-slate-50 p-5 dark:border-white/10 dark:bg-white/[0.03]">
              <span className="text-2xl">{h.emoji}</span>
              <p className="mt-2 text-sm font-bold text-slate-800 dark:text-white">{h.title}</p>
              <p className="mt-1 text-xs text-slate-500 dark:text-white/50">{h.desc}</p>
            </div>
          ))}
        </div>
